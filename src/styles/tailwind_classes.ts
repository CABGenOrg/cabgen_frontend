export const section_title = "font-bold 2xl:text-5xl md:text-5xl text-3xl";
export const section_subtitle = "font-light 2xl:text-4xl md:text-4xl text-2xl";
export const section_text = "my-5 2xl:text-xl md:text-lg text-base";
export const map_graph_title = "font-bold text-center my-4 2xl:text-4xl md:text-4xl text-2xl";
export const section_image = "object-cover w-5/6 max-w-full";
export const section_spacing =
  "xl:max-w-7xl xl:mx-auto lg:mx-32 md:mx-16 sm:mx-10 mx-7 md:py-3 py-5 sm:gap-2 gap-5";
export const section_btn =
  "bg-cabgen-200 hover:bg-cabgen-100 rounded-lg 2xl:py-2.5 py-2 2xl:px-6 px-6 2xl:text-lg text-lg text-white transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cabgen-200 focus-visible:ring-offset-2";
export const form_spacing =
  "w-full my-3 mx-auto flex flex-col justify-center items-center";
export const form_title =
  "font-semibold 2xl:text-4xl md:text-4xl text-2xl text-center tracking-tight text-gray-800";
export const input_class =
  "block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm placeholder:text-gray-400 focus:ring-2 focus:ring-cabgen-200 focus:outline-none 2xl:text-lg sm:text-base sm:leading-6";
export const label_class =
  "block 2xl:text-xl text-lg font-light tracking-wider leading-6 text-gray-900";
